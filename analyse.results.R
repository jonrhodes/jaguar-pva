...
for(current.expert in expert.vec) {
    ...
    for(current.expert.realization in expert.realization.vec) {

        cat('\nPloting results expert ID', current.expert, 'Expert realization', current.expert.realization)

        # pull out the result for the given expert
        model.output <- subset( model.output.all.experts, 
            expert.ID==current.expert & expert.realization==current.expert.realization)
        ...
        for(i in 1:num.reps ) {
            ...
            for( x in time.steps.vec ){
                ...
                for( cur.jcu in jcu.vec ) {
                    ...
                    pop.traj.jcu.array[x,i,cur.jcu] <- sum( subset( model.output, 
                                    stoch.realization==i & time==x & jcu==cur.jcu, select=total.pop ) )
